...
class Executor(job_api_pb2_grpc.JobServiceServicer):
    ...
    def UpdateModel(self, request_iterator, context):
        ...
        """
        logging.debug(f'[{self.this_rank}] Received GRPC UpdateModel request')
    ...
    def Train(self, request, context):
        ...
        """
        logging.debug(f'[{self.this_rank}] Received GRPC Train request')
    ...
    def Stop(self, request, context):
        ...
        """
        logging.debug(f'[{self.this_rank}] Received GRPC Stop request')
            ...
            if self.device == None:
                for i in range(torch.cuda.device_count()):
                    try:
                        ...
                        logging.info(f'{YELLOW_BOLD}[{self.this_rank}]{RESET} End up with cuda device ({self.device})')
                        break
    ...
    def init_control_communication(self, ps_ip, ps_port):
        ...
        logging.debug(f'[{self.this_rank}] Started GRPC server at {port}')
        ...
        while time.time() - start_time < 15 and not is_connected:
            try:
                ...
                logging.debug(f"[{self.this_rank}] Retrying connection to {ps_ip}:{ps_port}")
                pass
